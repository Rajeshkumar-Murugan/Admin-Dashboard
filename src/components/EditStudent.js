...
function EditStudent() {
    ...
    //     await fetch('hhttps://614eabf8b4f6d30017b482ba.mockapi.io/admin/'+params.id,{
    //         method:'PUT',
    //         headers:{
    ...
    let handleSave = async()=>{
        try {
            let res = await axios.put('https://614eabf8b4f6d30017b482ba.mockapi.io/admin/'+params.id,{name,email,mobile,degree,dept})
console.log(res);
            navigate('/all-student') 
        } catch (error) {
            console.log(error)
    ...
    let getData = async()=>{
        await fetch('https://614eabf8b4f6d30017b482ba.mockapi.io/admin/'+params.id,{method:'GET'})
        .then(response =>response.json())
        .then(data=>{
            setName(data.name)
            setEmail(data.email)
